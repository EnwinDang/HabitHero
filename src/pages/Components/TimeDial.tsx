...
export function TimeDial({ value, onChange, min, max, darkMode, accentColor, isRunning, timeLeft }: TimeDialProps) {
    ...
    return (
        <div
            ref={elementRef}
            className={`relative w-48 h-48 mb-6 ${!isRunning ? 'cursor-grab active:cursor-grabbing' : ''}`}
        ...
        >
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none select-none">
                <span className={`text-3xl font-bold font-mono tracking-tight ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                ...
                {!isRunning && (
                    <span className={`text-[10px] mt-1 opacity-50 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        DRAG TO SET
                    </span>
                )}
            </div>
