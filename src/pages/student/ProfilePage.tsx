                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1">
                        <div className="relative">
                            {/* Glow */}
                            <div className="absolute -inset-1 rounded-2xl blur-lg opacity-30" style={{ background: `linear-gradient(to bottom, ${accentColor}, #a855f7)` }}></div>

                            {/* Card */}
                            <div className={`relative bg-gradient-to-b ${theme.cardGradient} rounded-2xl p-6 text-center overflow-hidden transition-colors duration-300`} style={{ ...theme.borderStyle, borderWidth: '1px', borderStyle: 'solid' }}>
                                {/* Corner decorations */}
                                <div className="absolute top-0 left-0 w-12 h-12 border-l-2 border-t-2" style={{ borderColor: `${accentColor}80` }}></div>
                                <div className="absolute top-0 right-0 w-12 h-12 border-r-2 border-t-2" style={{ borderColor: `${accentColor}80` }}></div>
                                ...
                                <div className="grid grid-cols-3 gap-2 mt-4">
                                    <div className={`${theme.inputBg} rounded-lg p-2`}>
                                        <p className="text-yellow-400 font-bold">{user.stats.gold}</p>
                                        <p className={`${theme.textSubtle} text-xs`}>Gold</p>
                                    </div>
                                    <div className={`${theme.inputBg} rounded-lg p-2`}>
                                        <p className="font-bold" style={theme.accentText}>{user.stats.xp}</p>
                                        <p className={`${theme.textSubtle} text-xs`}>XP</p>
                                    </div>
                                    <div className={`${theme.inputBg} rounded-lg p-2`}>
                                        <p className="text-orange-400 font-bold">{user.stats.streak}</p>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className={`${theme.card} rounded-2xl p-6 transition-colors duration-300`} style={{ ...theme.borderStyle, borderWidth: '1px', borderStyle: 'solid' }}>
                            ...
                            <div className="space-y-4">
                                <InfoRow icon="👤" label="Hunter Name" value={user.displayName || "Hero"} darkMode={darkMode} />
                                <InfoRow icon="⚔️" label="Class" value="Shadow Monarch" darkMode={darkMode} />
                                <InfoRow icon="📧" label="Email" value={user.email || "hunter@shadow.com"} darkMode={darkMode} />
                                <InfoRow icon="📅" label="Awakened Since" value={memberSince} darkMode={darkMode} />
                            </div>
                        </div>
...
/* Info Row Component */
function InfoRow({ icon, label, value, darkMode }: { icon: string; label: string; value: string; darkMode: boolean }) {
    return (
        <div className="flex items-center gap-4 p-3 rounded-xl" style={{ backgroundColor: darkMode ? 'rgba(55, 65, 81, 0.2)' : 'rgba(243, 244, 246, 1)' }}>
            <span className="text-xl">{icon}</span>
            <div>
                <p className={darkMode ? 'text-gray-500' : 'text-gray-400'} style={{ fontSize: '0.875rem' }}>{label}</p>
