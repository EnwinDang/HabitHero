...
export default function StatsPage() {
    ...
    const completedEasy = tasks.filter(t => !t.isActive && t.difficulty === 'easy').length;
    const completedMedium = tasks.filter(t => !t.isActive && t.difficulty === 'medium').length;
    const completedHard = tasks.filter(t => !t.isActive && t.difficulty === 'hard').length;
    const completedExtreme = tasks.filter(t => !t.isActive && t.difficulty === 'extreme').length;
    ...
    return (
        <div className={`min-h-screen ${theme.bg} flex transition-colors duration-300`}>
            ...
            <main className="flex-1 p-8 overflow-y-auto">
                {/* Header */}
                <div className="mb-6">
                    <h2 className={`text-3xl font-bold ${theme.text}`}>Hero Statistics</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-5 text-white">
                        ...
                        <p className="text-orange-200 text-sm">{xp % maxXP} / {maxXP}</p>
                    </div>
                    ...
                    <div className={`${theme.card} rounded-2xl p-6`} style={{ ...theme.borderStyle, borderWidth: '1px', borderStyle: 'solid' }}>
                        ...
                        {totalTasks === 0 ? (
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center p-3 rounded-lg" style={{ backgroundColor: darkMode ? 'rgba(55, 65, 81, 0.3)' : 'rgba(243, 244, 246, 1)' }}>
                                    <span className={theme.textMuted}>Total Tasks</span>
                                    <span className={`font-bold ${theme.text}`}>{totalTasks}</span>
                                </div>
                                <div className="flex justify-between items-center p-3 rounded-lg" style={{ backgroundColor: darkMode ? 'rgba(34, 197, 94, 0.1)' : 'rgba(220, 252, 231, 1)' }}>
                                    <span className="text-green-500">Completed</span>
                                    <span className="font-bold text-green-500">{completedTasks}</span>
                                </div>
                                <div className="flex justify-between items-center p-3 rounded-lg" style={{ backgroundColor: darkMode ? 'rgba(59, 130, 246, 0.1)' : 'rgba(219, 234, 254, 1)' }}>
                                ...
                                {totalTasks > 0 && (
                                    <div className="mt-4">
                                        <div className="flex justify-between text-sm mb-1">
                                            ...
                                            <span style={theme.accentText}>{Math.round((completedTasks / totalTasks) * 100)}%</span>
                                        </div>
                ...
                <div className={`${theme.card} rounded-2xl p-6`} style={{ ...theme.borderStyle, borderWidth: '1px', borderStyle: 'solid' }}>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="text-center px-6 py-4 rounded-xl" style={{ backgroundColor: darkMode ? 'rgba(88, 28, 135, 0.3)' : 'rgba(243, 232, 255, 1)' }}>
                            <p className="text-4xl font-bold" style={theme.accentText}>{level}</p>
    ...
    return (
        <div>
            <div className="flex justify-between text-sm mb-1">
                ...
                <span className={theme.textMuted}>{completed}/{total}</span>
            </div>
