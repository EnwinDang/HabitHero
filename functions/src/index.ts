 ...
 */
async function updateModuleCompletionAchievements(uid: string, courseId: string, moduleId: string): Promise<void> {
  try {
    ...
    for (const taskDoc of tasksSnapshot.docs) {
      ...
      const userSubmission = Object.values(submissions).find((sub: any) => 
        sub.studentId === uid && sub.status === "approved"
      );
    ...
    await Promise.all(
      moduleAchievements.map(async (achievement: any) => {
 ...
 */
async function updateWorldCompletionAchievements(uid: string, worldId: string): Promise<void> {
  try {
    ...
    if (isWorldComplete) {
      ...
      await Promise.all(
        worldAchievements.map(async (achievement: any) => {
          const achievementId = String(achievement.achievementId || "").toLowerCase();
  ...
  try {
    ...
    await userRef.set({
      stats: {
        ...
        focusSessionsCompleted: FieldValue.increment(1), // Track total completed focus sessions for achievements
