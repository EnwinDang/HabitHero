 ...
 */
async function calculateTotalStatsForUser(user) {
    ...
    const equippedTotals = Object.fromEntries(aggregateKeys.map(k => [k, 0]));
    const resolveVal = (key, statsObj, buffsObj) => {
        const s = statsObj || {};
        const b = buffsObj || {};
        if (key === "critChance")
            return Number(s.crit ?? b.crit ?? s.critChance ?? b.critChance ?? 0);
        if (key === "magicAttack")
            return Number(s.magic ?? b.magic ?? s.magicAttack ?? b.magicAttack ?? 0);
        if (key === "magicResist")
            return Number(s.magicRes ?? b.magicRes ?? s.magicResist ?? b.magicResist ?? 0);
        if (key === "goldBonus")
            return Number(s.gold ?? b.gold ?? s.goldBonus ?? b.goldBonus ?? 0);
        if (key === "xpBonus")
            return Number(s.xp ?? b.xp ?? s.xpBonus ?? b.xpBonus ?? 0);
        return Number(s[key] ?? b[key] ?? 0);
    };
    const resolveFromInventory = (eqId) => {
        const match = items.find((i) => i.itemId === eqId || i.id === eqId || i.instanceId === eqId);
        return match || null;
    };
    const equippedIds = [
 ...
 */
async function getStaminaConfig() {
    try {
 ...
 */
async function getMonsterTier(worldId, stage, monsterId) {
...
async function calculateCurrentStamina(currentStamina, maxStamina, lastRegen, regenRateMinutes) {
    const now = Date.now();
...
app.get("/auth/me", requireAuth, async (req, res) => {
    try {
        ...
        if (user.stats?.stamina === undefined || user.stats?.maxStamina === undefined) {
            ...
            const { stamina, lastRegen: newLastRegen } = await calculateCurrentStamina(currentStamina, maxStamina, lastRegen, regenRateMinutes);
        ...
        try {
            ...
            userData.stats = { ...userData.stats, totalStats };
        }
        catch (calcErr) {
...
app.post("/tasks/:taskId/submissions", requireAuth, async (req, res) => {
    try {
        ...
        const submission = {
            ...
            status: "pending",
            teacherComment: null,
            createdAt: existingSubmission ? existingSubmission.createdAt : now,
            updatedAt: now,
            courseId,
            moduleId,
            taskId,
        ...
        if (worldId && stage) {
            try {
                ...
                if (stageTypeSnap.exists) {
                    ...
                    const stageStructure = stageStructureConfig || {};
...
app.get("/users/:uid/inventory", requireAuth, async (req, res) => {
    try {
        ...
        const user = userSnap.data() || {};
        const rawItems = user.inventory?.inventory?.items || [];
        ...
        const resolveDetail = async (item, idx, forceEquipped = false) => {
            const base = {
        ...
        const equippedIds = [];
        if (equippedObj.weapon)
            equippedIds.push({ id: equippedObj.weapon, slot: "weapon" });
        Object.entries(equippedObj.armor || {}).forEach(([slot, id]) => id && equippedIds.push({ id: id, slot }));
        Object.entries(equippedObj.pets || {}).forEach(([slot, id]) => id && equippedIds.push({ id: id, slot }));
        Object.entries(equippedObj.accessoiries || {}).forEach(([slot, id]) => id && equippedIds.push({ id: id, slot }));
...
app.post("/users/:uid/stamina/consume", requireAuth, async (req, res) => {
    try {
        ...
        const lastRegen = user.stats?.lastStaminaRegen;
        // Calculate current stamina with regeneration
        const { stamina: staminaBefore, lastRegen: newLastRegen } = await calculateCurrentStamina(currentStamina, maxStamina, lastRegen, regenRateMinutes);
...
app.get("/combat/monster-stats/:worldId/:stage/:userLevel", async (req, res) => {
    try {
        ...
        const itemsCount = equippedItemsCount ? parseInt(equippedItemsCount) : 0;
        ...
        const worldMonstersSnap = await db.collection(`worlds/${worldId}/monsters`).doc(monsterId).get();
        if (worldMonstersSnap.exists) {
            monsterData = worldMonstersSnap.data();
        }
        else {
            // Fallback: try general monsters collection
            const generalMonsterSnap = await db.collection("monsters").doc(monsterId).get();
                ...
                for (const collectionName of itemCollections) {
                    try {
                        ...
                        const items = itemsSnap.docs
                            .map(doc => ({ itemId: doc.id, ...doc.data() }))
                            .filter((item) => item.isActive !== false && item.rarity === selectedRarity);
                        if (items.length > 0) {
                    ...
                    for (const fallbackRarity of fallbackRarities) {
                        ...
                        for (const collectionName of itemCollections) {
                            try {
                                ...
                                const items = itemsSnap.docs
                                    .map(doc => ({ itemId: doc.id, ...doc.data() }))
                                    .filter((item) => item.isActive !== false && item.rarity === fallbackRarity);
                                if (items.length > 0) {
                                    allItemsOfRarity.push(...items.map(item => ({ ...item, collection: collectionName })));
                                }
                            }
...
app.post("/pomodoro/session-completed", async (req, res) => {
    try {
        ...
        await userRef.set({
            stats: {
                todaysSessions: needsReset ? 1 : firestore_1.FieldValue.increment(1),
